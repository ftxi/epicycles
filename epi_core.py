...
def mp4(filename, size, speed, r, p, n, l, line_min=5, filter_zero=True) :
    ...
    size *= DIFF
    N = len(n)
    ts = np.linspace(0, 2*np.pi, 4000, endpoint=False)
    imgbase = Image.new('RGB', (2*size, 2*size), (255, 255, 255))
    drawbase = ImageDraw.Draw(imgbase)
    ...
    try :    
        writer = imageio.get_writer(filename, fps=64)
        i = 0
        ...
        print i
        writer.close()
    except imageio.NeedDownloadError:
        print("Oops. It is said that 'Need ffmpeg exe' by imageio module, which I am trying to. Wait a minute.")
